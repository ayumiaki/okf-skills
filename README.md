...
| [`skills/validate/`](skills/validate/) | **Deterministic §9 conformance check.** Zero-config — `uv run skills/validate/scripts/okf_validate.py <path> --strict`. Catches missing frontmatter, wrong types, broken cross-links. |
| [`skills/visualize/`](skills/visualize/) | **Self-contained interactive graph.** Renders any bundle to a single `viz.html` — nodes coloured by type, clickable, searchable, deep-linkable. |
